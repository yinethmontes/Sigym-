...
if(@$add){	
    ...
    $sql= mysql_fetch_array(mysql_query("select max(MOD_IDE)+1 as next from modalidad"));
    $next= $sql['next'];

	if($db->consulta("INSERT INTO $tabla (MOD_IDE,MOD_NOM,MOD_DES) VALUES ('".$next."','".$txt_nom."','".$txt_des."')")){		
		$id = mysql_insert_id();
		if($ctr=="apli"){ 
...
if(@$mod){	
	$sel="SELECT * FROM $tabla WHERE MOD_IDE = '$mod' ";
	$row=$db->ver($sel);
}

if(@$edi){
	header("Content-Type: text/html;charset=utf-8");
	if($db->consulta("update $tabla set MOD_NOM = '".$txt_nom."', MOD_DES = '".$txt_des."'  WHERE MOD_IDE = $id")){
...
if(@$eli){
	...
	if($db->consulta("delete from modalidad where MOD_IDE = $eli")){
...
if(@$grilla){
	...
	if($buscar){ $where = "WHERE a.MOD_NOM LIKE '%$buscar%'"; }	
	if(!$sidx) $sidx =1; $row=$db->ver("SELECT COUNT(*) AS count FROM $tabla a $where"); 
	$count = $row['count'];	
	
	if( $count >0 ){ $total_pages = ceil($count/$limit);}else{ $total_pages = 0;} //para paginar
	
	if ($page > $total_pages) $page=$total_pages; $start = $limit*$page - $limit;
	$que= "SELECT a.MOD_IDE as id, a.MOD_NOM as modalidad, a.MOD_DES as descripcion FROM $tabla a $where
	...
	while($row = $db->listar($consulta)){		
		echo "<row id='".$row[id]."'>"; 
		echo "<cell>".utf8_encode($row[id])."</cell>";
...
<a href="listados_pdf/pdf_modalidades.php" target="_blank"><img src="images/logopdf.png" title="Exportar a PDF" border="0"/></a>
...
$().ready(function() {
  ...
  $("#gri1").jqGrid({
		url:'?grilla=1',		
		colNames:['Código Modalidad','Nombre Modalidad','Opciones'],
		colModel:[
			{name:'id',index:'id', width:80,align:"center"},
			{name:'Modalidad',index:'modalidad', width:250},
      ...
      <table width="95%" border="0" align="center" id="tabla_form">
        <tr>
          ...
          <td width="80%" colspan="2"><strong>
            <?php /*$consecutivo = "select count(MOD_IDE) as numero from $tabla"; $ejecuta = mysql_query($consecutivo); 
        ...
        <tr>
          <td>Descripción:</td>
          <td colspan="2"><textarea name="txt_des" cols="90" rows="5" id="txt_des"></textarea></td>
        </tr>
...
<div id="conte" class="ui-widget-content ui-corner-all" >
  ...
  <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
    <table width="95%" border="0" align="center" id="tabla_form2">
      <tr>
        <td>Codigo:</td>
        <td width="80%" colspan="2"><strong><?php echo $row['MOD_IDE']?></strong></td>
      </tr>
      <tr>
        <td width="20%">Modalidad:</td>
        <td colspan="2"><input name="txt_nom" type="text" id="txt_nom" value="<?php echo $row['MOD_NOM']?>" size="80" maxlength="200" /></td>
      </tr>
      <tr>
        <td>Descripción:</td>
        <td colspan="2"><textarea name="txt_des" cols="90" rows="5" id="txt_des"><?php echo $row['MOD_DES']?></textarea></td>
      </tr>
      
      
      
      <tr>
        <td colspan="3" align="center"><input name="id" type="hidden" id="id" value="<?php echo $row['MOD_IDE']?>" />
          <input type="hidden" name="ctr" id="ctr" />
	...
	<div class="container">
		<section>
			...
			<table id="example" class="display" cellspacing="0" width="100%">
                ...
                <thbody>
                <?php 
                 $sql="SELECT a.MOD_IDE as id, a.MOD_NOM as modalidad FROM $tabla a";
